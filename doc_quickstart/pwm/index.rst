...
The test application uses 4 cores containing the following components:-
   ...
   #. PWM capture (captures raw PWM data via 'loopback')
...
The test application uses the following channels:-
   ...
   #. c_chk[]: An array of channels for transmitting raw PWM data from the Capture to the Checker core
...
The generator runs through a set of tests, these are specified formally as a *test vector* and transmitted to the test checker. For each test the generator creates the required pulse-width and sends this to the PWM Client. The PWM Client in turn sends the pulse-width to the PWM Server. The PWM Server converts the pulse-width into a PWM wave-train and drives this onto the output pins. The PWM Capture samples its input pins every 32-bits, if a new sample is detected this is transmitted to the PWM Checker. The PWM Checker stores the raw PWM data in a buffer until such time when it can be checked. The PWM test checker also reads the specification in the received test vector. The received PWM data is then checked for correctness against the test vector specification.
...
----------------
   ...
   #. Review the ``main.xc`` and note that main() runs 4 tasks on 4 logical cores in parallel.
         ...
         * ``capture_pwm_server_data()`` captures the raw PWM data which has been looped back onto a set of input pins, and transmits this over channel c_chk
         * ``check_pwm_server_data()`` receives raw PWM data over channel ``c_chk``, checks it, and displays the results. ``gen_all_pwm_test_data()`` and ``check_all_pwm_server_data()`` both produce display information in parallel. 
